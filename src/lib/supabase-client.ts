...
export class SupabaseClient {
    ...
    async fetchJobs(queryString: string | null): Promise<Job[]> {
        let searchPromise;
        if (queryString) {
            searchPromise = this.supabaseClient.from('jobs').select().textSearch('fts', queryString);
        } else {
        ...
        if (error) {
            console.log(`Error response from Supabase: ${error.code} ${error.message} ${error.hint} ${error.details}`);
        }
